              ...
              const uint32_t addr[8])
{
    ...
    sha256_update(&sha2_state, (unsigned char *)addr, 22);
    sha256_update(&sha2_state, ctx->sk_seed, SPX_N);
    sha256_final(&sha2_state, outbuf);
    memcpy(out, outbuf, SPX_N);
}

void gen_message_random_spx_16(unsigned char *R, const unsigned char *sk_prf,
                               ...
                               const spx_ctx *ctx)
{
    ...
    unsigned char inner_hash[SPX_SHA256_OUTPUT_BYTES];
    SHA256_CTX sha2_state;
    int i;
    ...
    sha256_init(&sha2_state);
    sha256_update(&sha2_state, buf, SPX_SHA256_BLOCK_BYTES);
    sha256_update(&sha2_state, optrand, SPX_N);
    sha256_update(&sha2_state, m, mlen);
    sha256_final(&sha2_state, inner_hash);
    ...
    sha256_init(&sha2_state);
    sha256_update(&sha2_state, buf, SPX_SHA256_BLOCK_BYTES);
    sha256_update(&sha2_state, inner_hash, SPX_SHA256_OUTPUT_BYTES);
    sha256_final(&sha2_state, buf);

    memcpy(R, buf, SPX_N);
}

void gen_message_random_spx_24(unsigned char *R, const unsigned char *sk_prf,
                               const unsigned char *optrand,
                               const unsigned char *m, unsigned long long mlen,
                               const spx_ctx *ctx)
{
    ...
    unsigned char inner_hash[SPX_SHA512_OUTPUT_BYTES];
    ...
    sha512_update(&sha2_state, buf, SPX_SHA512_BLOCK_BYTES);
    sha512_update(&sha2_state, optrand, SPX_N);
    sha512_update(&sha2_state, m, mlen);
    sha512_final(&sha2_state, inner_hash);
    ...
    sha512_init(&sha2_state);
    sha512_update(&sha2_state, buf, SPX_SHA512_BLOCK_BYTES);
    sha512_update(&sha2_state, inner_hash, SPX_SHA512_OUTPUT_BYTES);
    sha512_final(&sha2_state, buf);

    memcpy(R, buf, SPX_N);
}

void gen_message_random_spx_32(unsigned char *R, const unsigned char *sk_prf,
                               const unsigned char *optrand,
                               const unsigned char *m, unsigned long long mlen,
                               const spx_ctx *ctx)
{
                        ...
                        const spx_ctx *ctx)
{
    ...
    if (SPX_N == 16) {
        return gen_message_random_spx_16(R, sk_prf, optrand, m, mlen, ctx);
    }
    if (SPX_N == 24) {
        return gen_message_random_spx_24(R, sk_prf, optrand, m, mlen, ctx);
    }
    if (SPX_N == 32) {
        return gen_message_random_spx_32(R, sk_prf, optrand, m, mlen, ctx);
    }
...
void hash_message_spx_16(unsigned char *digest, uint64_t *tree, uint32_t *leaf_idx,
...
#define SPX_DGST_BYTES (SPX_FORS_MSG_BYTES + SPX_TREE_BYTES + SPX_LEAF_BYTES)
    ...
    sha256_init(&sha2_state);
    sha256_update(&sha2_state, R, SPX_N);
    sha256_update(&sha2_state, pk, SPX_PK_BYTES);
    sha256_update(&sha2_state, m, mlen);
    sha256_final(&sha2_state, seed + 2*SPX_N);

    // H_msg: MGF1-SHA-256(R || PK.seed || seed)
    memcpy(seed, R, SPX_N);
    memcpy(seed + SPX_N, pk, SPX_N);

    mgf1_256(bufp, SPX_DGST_BYTES, seed, 2*SPX_N + SPX_SHA256_OUTPUT_BYTES);
...
void hash_message_spx_24(unsigned char *digest, uint64_t *tree, uint32_t *leaf_idx,
                         const unsigned char *R, const unsigned char *pk,
                         const unsigned char *m, unsigned long long mlen,
                         const spx_ctx *ctx) {
    (void)ctx;
...
#define SPX_DGST_BYTES (SPX_FORS_MSG_BYTES + SPX_TREE_BYTES + SPX_LEAF_BYTES)
    ...
    sha512_init(&sha2_state);
    sha512_update(&sha2_state, R, SPX_N);
    sha512_update(&sha2_state, pk, SPX_PK_BYTES);
    sha512_update(&sha2_state, m, mlen);
    sha512_final(&sha2_state, seed + 2*SPX_N);

    // H_msg: MGF1-SHA-512(R || PK.seed || seed)
    memcpy(seed, R, SPX_N);
    memcpy(seed + SPX_N, pk, SPX_N);

    mgf1_512(bufp, SPX_DGST_BYTES, seed, 2*SPX_N + SPX_SHA512_OUTPUT_BYTES);
...
void hash_message_spx_32(unsigned char *digest, uint64_t *tree, uint32_t *leaf_idx,
                         const unsigned char *R, const unsigned char *pk,
                         const unsigned char *m, unsigned long long mlen,
                         const spx_ctx *ctx) {
    (void)ctx;
...
{
    if (SPX_N == 16) {
        return hash_message_spx_16(digest, tree, leaf_idx, R, pk, m, mlen, ctx);
    }
    if (SPX_N == 24) {
        return hash_message_spx_24(digest, tree, leaf_idx, R, pk, m, mlen, ctx);
    }
    if (SPX_N == 32) {
        return hash_message_spx_32(digest, tree, leaf_idx, R, pk, m, mlen, ctx);
    }
